...
<body class="bg-gray-100 min-h-screen">
    ...
    <nav class="bg-white shadow-sm border-b px-6 py-4 flex justify-between items-center sticky top-0 z-50">
        ...
        <div class="flex gap-3">
            <a href="{{ route('home') }}" class="px-4 py-2 text-sm text-gray-600 hover:text-green-600 bg-gray-50 rounded-lg border border-gray-200 transition">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-white p-5 rounded-xl shadow-sm border-l-4 border-blue-500 flex justify-between items-center">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-800">{{ $data->count() }} <span class="text-sm font-normal">Sampel</span></h3>
                </div>
            ...
            <div class="bg-white p-5 rounded-xl shadow-sm border-l-4 border-red-500 flex justify-between items-center">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-gray-800">{{ $sampah->count() }} <span class="text-sm font-normal">File</span></h3>
                </div>
            ...
            <div class="bg-white p-5 rounded-xl shadow-sm border-l-4 border-green-500 flex flex-col justify-center">
                ...
                <a href="{{ route('admin.export') }}" class="bg-green-600 hover:bg-green-700 text-white text-center py-2 rounded-lg text-sm font-bold transition shadow-sm flex items-center justify-center gap-2">
                        ...
                        <tr class="bg-white border-b hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <div class="h-16 w-16 rounded-lg overflow-hidden border border-gray-200 shadow-sm group relative">
                                    <img src="{{ asset($item->image_path) }}" class="object-cover w-full h-full transform group-hover:scale-110 transition duration-300" alt="Padi">
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                <form action="{{ route('admin.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus data ini?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:bg-red-50 p-2 rounded-lg transition" title="Hapus Data">
    ...
    <tr class="bg-white border-b hover:bg-red-50 transition">
        ...
        <td class="px-6 py-4 text-center">
            <form action="{{ route('admin.destroyFailed', $item->id) }}" method="POST" onsubmit="return confirm('Hapus permanen data sampah ini?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="text-red-500 hover:bg-red-100 p-2 rounded-lg transition border border-red-200" title="Hapus Permanen">
